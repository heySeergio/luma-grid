...
const features = [
  ...
  {
    title: 'Frases con IA y voz creíble',
    description:
...
const modules = [
  ...
  'Voz del sistema y voces naturales y realistas con caché',
]

type Props = {
  /** En la copia archivada conviene false para no duplicar JSON-LD. Por defecto true (página pública). */
  includeJsonLd?: boolean
  /**
   * Modo «próximamente» (p. ej. raíz `/`): sin acceso a tablero, admin ni registro desde esta página.
   */
  comingSoon?: boolean
}

/**
 * Landing completa tal como estaba en la raíz (copia única de contenido).
 * Se usa en `/` y en la ruta archivada oculta a buscadores.
 */
export default function LandingPageOriginal({ includeJsonLd = true, comingSoon = false }: Props) {
  const showJsonLd = includeJsonLd && !comingSoon
  return (
    <main className="min-h-screen select-none bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 [&_a]:select-text [&_button]:select-text [&_input]:select-text [&_select]:select-text [&_textarea]:select-text">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-5">
                <h1 className="text-[2.15rem] font-black leading-[1.08] tracking-tight text-slate-950 dark:text-white sm:text-5xl lg:text-[3.25rem] lg:leading-[1.05]">
                  {comingSoon ? (
                    <>
                      <span className="block">Luma Grid</span>
                ...
                <p className="max-w-lg text-base leading-relaxed text-slate-600 dark:text-slate-300 md:text-lg">
                  {comingSoon
                    ? 'Estamos terminando el comunicador AAC: pictogramas, frases con IA, voz y modo offline. El acceso público al tablero y a las cuentas se activará en una próxima versión. Mientras tanto, aquí puedes conocer qué ofrecerá Luma Grid.'
                    : 'Construye frases con pictogramas, conjúgalas con ayuda inteligente y escúchalas al momento. Un solo flujo para comunicarse con autonomía, en casa o fuera de casa.'}
                </p>
              </div>
              ...
              <ul className="flex flex-col gap-3 border-t border-slate-200/80 pt-8 text-sm text-slate-600 dark:border-white/10 dark:text-slate-400 sm:flex-row sm:flex-wrap sm:gap-x-8 sm:gap-y-2">
                <li className="flex items-center gap-2">
                  ...
                  Voz del sistema y voces naturales y realistas
                </li>
                <li className="flex items-center gap-2">
              ...
              <div className="relative overflow-hidden rounded-[1.75rem] border border-slate-200/90 bg-white/90 shadow-2xl shadow-slate-900/10 ring-1 ring-black/5 backdrop-blur dark:border-white/12 dark:bg-slate-900/90 dark:shadow-black/40 dark:ring-white/10">
                <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 dark:border-white/10">
                  <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
                    {comingSoon ? 'Anticipo visual' : 'Vista previa'}
                  </span>
        ...
        <div className="mx-auto w-full max-w-6xl px-6 py-16 md:px-10">
          ...
          <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-5">
            {modules.map(module => (
              <li
                key={module}
                className="group rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm font-medium leading-snug text-slate-700 shadow-sm transition duration-300 ease-out hover:-translate-y-1 hover:border-indigo-300/70 hover:shadow-lg hover:shadow-indigo-500/10 dark:border-white/10 dark:bg-white/5 dark:text-slate-200 dark:shadow-none dark:hover:border-indigo-500/35 dark:hover:shadow-indigo-500/20"
              >
                {module}
              </li>
            ))}
        ...
        <div className="mx-auto w-full max-w-6xl px-6 py-16 md:px-10">
          <div className="mb-10 text-center md:text-left">
            ...
            <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-600 dark:text-slate-400 md:text-base">
              {comingSoon
                ? 'Referencia de planes previstos. La contratación y el registro estarán disponibles cuando abramos el servicio al público.'
                : 'Empieza gratis sin tarjeta. Escala a voces naturales y realistas —de alta calidad— y más tableros cuando lo necesites.'}
            </p>
          </div>
        ...
        <div className="rounded-3xl border border-indigo-300/30 bg-indigo-500/10 p-8 md:p-10 dark:border-indigo-300/30">
          ...
          {comingSoon ? (
            <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-700 dark:text-indigo-100/90 md:text-base">
              Luma Grid será una PWA: podrás instalarla en la pantalla de inicio y abrirla como aplicación en{' '}
              <strong className="font-semibold text-slate-900 dark:text-white">cualquier dispositivo</strong>
              —móviles y tablets <strong className="font-semibold text-slate-900 dark:text-white">Android</strong> e{' '}
              <strong className="font-semibold text-slate-900 dark:text-white">iOS</strong>, y ordenadores{' '}
              <strong className="font-semibold text-slate-900 dark:text-white">Windows</strong>,{' '}
              <strong className="font-semibold text-slate-900 dark:text-white">macOS</strong> o{' '}
              <strong className="font-semibold text-slate-900 dark:text-white">Linux</strong>— usando un navegador moderno. Las guías paso a paso
              se publicarán cuando el servicio esté disponible.
            </p>
          ) : (
            <>
              <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-700 dark:text-indigo-100/90 md:text-base">
      ...
      <SiteFooter comingSoon={comingSoon} />
    </main>
  )
}
